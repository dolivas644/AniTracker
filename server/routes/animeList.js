...
router.get('/', async (req, res) => {
    ...
    try {
      const itemList = await db.any(`SELECT a.*, u.complete=false, u.id as list_id FROM anime as a JOIN user_anime_list as u ON a.mal_id = u.anime_id WHERE u.user_id=$1 AND
      u.complete = FALSE`, [user_id]);
      res.send(itemList);
    } catch (e) {
      console.log(e);
